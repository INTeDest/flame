...
В разработке игр компонент — это самодостаточная единица, инкапсулирующая определённую часть поведения или визуализации игры. Flame использует [Систему компонентов Flame](../game.md) (FCS), в которой каждый объект вашей игры (игроки, враги, фон, элементы интерфейса) является компонентом. Это упрощает создание и поддержку игр, поскольку каждый фрагмент логики находится в своём классе, а компоненты можно свободно объединять в дерево, подобно [дереву виджетов Flutter](https://docs.flutter.dev/get-started/fundamentals/widgets).

- [Компонент позиции](position_component.md)
- [Спрайтовые компоненты](sprite_components.md)
- [Параллакс-компонент](parallax_component.md)
- [Компоненты фигур](shape_components.md)
- [Служебные компоненты](utility_components.md)

```{include} ../diagrams/component.md
```

Диаграмма может показаться сложной, но не беспокойтесь — всё не так страшно, как выглядит.


## Component

Все компоненты наследуются от класса `Component` и могут содержать другие `Component` в качестве дочерних. Это основа так называемой Системы компонентов Flame (FCS).

Дочерние компоненты можно добавлять либо методом `add(Component c)`, либо напрямую в конструкторе.
...
Разумеется, `Component()` здесь может быть любым подклассом `Component`.

Каждый `Component` имеет несколько методов, которые можно при желании реализовать; они используются классом `FlameGame`.
...
Метод `onGameResize` вызывается при каждом изменении размера экрана, а также когда компонент добавляется в дерево компонентов, перед `onMount`.

Метод `onParentResize` аналогичен: он также вызывается при монтировании компонента в дерево, а также при каждом изменении размера родительского компонента.

Метод `onRemove` можно переопределить, чтобы выполнить код перед удалением компонента из игры. Он выполняется только один раз, даже если компонент удаляется и через метод `remove` родителя, и через собственный метод `remove` компонента.

Метод `onLoad` можно переопределить для выполнения асинхронного кода инициализации компонента, например загрузки изображения. Этот метод выполняется до `onGameResize` и `onMount`. Гарантированно выполняется только один раз за время жизни компонента, так что его можно рассматривать как «асинхронный конструктор».

Метод `onMount` выполняется каждый раз при монтировании компонента в игровое дерево. Это означает, что здесь не следует инициализировать `late final` переменные, поскольку метод может выполняться несколько раз в течение жизни компонента. Он будет выполнен только в том случае, если родитель уже смонтирован. Если родитель ещё не смонтирован, метод будет ждать в очереди (это не повлияет на остальную работу движка).

Метод `onChildrenChanged` можно переопределить, если требуется отслеживать изменения в дочерних компонентах родителя. Он вызывается при каждом добавлении или удалении дочернего компонента у родителя (включая смену родителя). Параметры содержат целевой дочерний компонент и тип изменения (`added` или `removed`).

Метод `onHotReload` вызывается у каждого компонента в дереве при горячей перезагрузке Flutter (только в режиме отладки). Переопределите его для перезагрузки ресурсов, пересчёта кешированных значений или выполнения других действий в ответ на изменения кода в процессе разработки. Уведомление автоматически распространяется на все дочерние компоненты, которые загружены или загружаются, поэтому в переопределении необходимо вызвать `super.onHotReload()`:
...
class MyComponent extends Component {
  ...
  void onHotReload() {
    super.onHotReload();
    // Повторно прочитать значения, которые могли измениться в исходном коде.
    _cachedValue = _computeExpensiveValue();
  }
}
```

Состояние жизненного цикла компонента можно проверить с помощью ряда геттеров:

- `isLoaded`: возвращает `bool` с текущим состоянием загрузки.
- `loaded`: возвращает `Future`, который завершится после завершения загрузки компонента.
- `isMounted`: возвращает `bool` с текущим состоянием монтирования.
- `mounted`: возвращает `Future`, который завершится после завершения монтирования компонента.
- `isRemoved`: возвращает `bool` с текущим состоянием удаления.
- `removed`: возвращает `Future`, который завершится после удаления компонента.


### Приоритет

Во Flame каждый `Component` имеет свойство `int priority`, которое определяет порядок сортировки компонента среди дочерних компонентов его родителя. В других языках и фреймворках это иногда называют `z-index`. Чем выше установлен `priority`, тем ближе к зрителю будет отображаться компонент, поскольку он рендерится поверх компонентов с более низким приоритетом, отрисованных ранее.

Если вы добавите два компонента и, например, установите одному из них приоритет 1, то этот компонент будет отрисован поверх другого (если они перекрываются), поскольку приоритет по умолчанию равен 0.

Все компоненты принимают `priority` как именованный аргумент, поэтому если приоритет известен на этапе компиляции, его можно передать в конструктор.
...
Чтобы обновить приоритет компонента, присвойте ему новое значение, например `component.priority = 2`, и оно будет применено в текущем тике перед этапом рендеринга.

В следующем примере мы сначала инициализируем компонент с приоритетом 1, а затем при касании меняем его приоритет на 2:
...
Иногда полезно оборачивать одни компоненты внутрь других, например группируя визуальные компоненты в иерархию. Это можно сделать, добавляя дочерние компоненты к любому компоненту, например к `PositionComponent`.

Если у компонента есть дочерние компоненты, то при каждом обновлении и рендеринге родителя все дочерние компоненты также обновляются и рендерятся с теми же условиями.

Пример, в котором видимость двух компонентов управляется через обёртку:
...
class GameOverPanel extends PositionComponent {
  ...
  @override
  void onLoad() {
    // GameOverText — это Component
    final gameOverText = GameOverText(spriteImage);
    // GameOverRestart — это SpriteComponent
  ...
  void render(Canvas canvas) {
    if (visible) {
    } // Если не видима, ни один из дочерних элементов не будет отрисован
  }
}
```

Существует два способа добавления дочерних компонентов. Во-первых, методы `add()`, `addAll()` и `addToParent()`, которые можно использовать в любой момент игры. Обычно дочерние компоненты создаются и добавляются в методе `onLoad()` компонента, но добавление новых дочерних компонентов по ходу игры также распространено.

Второй способ — использование параметра `children:` в конструкторе компонента. Этот подход ближе к стандартному API Flutter:
...
Эти два подхода можно свободно комбинировать: компоненты, указанные в конструкторе, добавляются первыми, а затем — все остальные.

Обратите внимание, что дочерние компоненты, добавленные любым из методов, гарантированно будут доступны только после их загрузки и монтирования. Мы можем лишь гарантировать, что они появятся в списке `children` в том же порядке, в котором были запланированы к добавлению.


### Доступ к World из компонента

Если компонент имеет предка типа `World` и нуждается в доступе к этому объекту `World`, можно использовать примесь `HasWorldReference`.
...
Если попытаться обратиться к `world` из компонента, не имеющего предка `World` нужного типа, будет выброшена ошибка утверждения.


### Обеспечение наличия определённого родителя у компонента

Когда компонент должен быть добавлен к родителю определённого типа, можно использовать примесь `ParentIsA`, чтобы гарантировать строго типизированного родителя.
...
class MyComponent extends Component with ParentIsA<MyParentComponent> {
  @override
  void onLoad() {
    // parent имеет тип MyParentComponent
    print(parent.myValue);
  }
}
```

Если попытаться добавить `MyComponent` к родителю, который не является `MyParentComponent`, будет выброшена ошибка утверждения.


### Обеспечение наличия определённого предка у компонента

Когда компонент должен иметь определённый тип предка где-то в дереве компонентов, можно использовать примесь `HasAncestor` для обеспечения этой связи.

Примесь предоставляет поле `ancestor` указанного типа.
...
class MyComponent extends Component with HasAncestor<MyAncestorComponent> {
  @override
  void onLoad() {
    // ancestor имеет тип MyAncestorComponent.
    print(ancestor.myValue);
  }
}
```

Если попытаться добавить `MyComponent` в дерево, не содержащее `MyAncestorComponent`, будет выброшена ошибка утверждения.


### Ключи компонентов

Компоненты могут иметь идентификационный ключ, позволяющий извлекать их из дерева компонентов из любой точки.

Чтобы зарегистрировать компонент с ключом, просто передайте ключ в аргумент `key` конструктора компонента:
...
Затем в другом месте дерева компонентов его можно получить:

```dart
flameGame.findByKey(ComponentKey.named('player'));
```

Существует два типа ключей: `unique` и `named`. Уникальные ключи основаны на равенстве экземпляров ключа, то есть:
...
Именованные ключи основаны на имени, которое они получают:
...
При использовании именованных ключей для получения компонента также можно использовать вспомогательный метод `findByKeyName`:
...
### Запрос дочерних компонентов

Добавленные к компоненту дочерние компоненты находятся в `QueryableOrderedSet` с именем `children`. Для запроса компонентов определённого типа в этом наборе можно использовать функцию `query<T>()`. По умолчанию `strictMode` в наборе `children` равен `false`, но если установить его в `true`, то перед запросом его необходимо зарегистрировать с помощью `children.register`.

Если на этапе компиляции известно, что позже будет выполняться запрос определённого типа, рекомендуется зарегистрировать запрос независимо от того, установлен `strictMode` в `true` или `false`, так как это даёт некоторые преимущества в производительности. Вызов `register` обычно делается в `onLoad`.
...
В примере выше регистрируется запрос для `PositionComponent`. Пример использования зарегистрированного запроса:
...
### Запрос компонентов в определённой точке экрана

Метод `componentsAtPoint()` позволяет проверить, какие компоненты были отрисованы в заданной точке экрана. Возвращаемое значение — итерируемая коллекция компонентов, но вы также можете получить координаты исходной точки в локальном координатном пространстве каждого компонента, передав изменяемый `List<Vector2>` в качестве второго параметра.

Итератор возвращает компоненты в порядке от переднего плана к заднему, то есть сначала идут компоненты на переднем плане, затем — на заднем.

Этот метод может возвращать только компоненты, реализующие метод `containsLocalPoint()`. `PositionComponent` (являющийся базовым для многих компонентов во Flame) предоставляет такую реализацию. Однако если вы определяете пользовательский класс, производный от `Component`, вам придётся реализовать `containsLocalPoint()` самостоятельно.

Пример использования `componentsAtPoint()`:
...
Обычно рекомендуемый способ скрыть или показать компонент — добавить или удалить его из дерева с помощью методов `add` и `remove`.

Однако добавление и удаление компонентов из дерева вызывает шаги жизненного цикла (такие как вызов `onRemove` и `onMount`). Это также асинхронный процесс, и при быстром последовательном удалении и добавлении компонента необходимо дождаться завершения удаления перед повторным добавлением.

```dart
/// Пример обработки быстрого удаления и добавления дочернего компонента
void show() async {
  // Необходимо сначала дождаться Future [removed] на случай,
  // если компонент ещё удаляется.
...
Альтернативный способ показать или скрыть компонент — использовать примесь `HasVisibility`, которую можно применить к любому классу, наследующему от `Component`. Эта примесь добавляет свойство `isVisible`. Просто установите `isVisible = false`, чтобы скрыть компонент, и `true` — чтобы снова показать, не удаляя его из дерева. Это влияет на видимость компонента и всех его потомков.

```dart
/// Пример реализации HasVisibility
class MyComponent extends PositionComponent with HasVisibility {}

/// Использование свойства isVisible
...
Примесь влияет только на то, отрисовывается ли компонент, и не влияет на другое поведение.

```{note}
Важно! Даже когда компонент не видим, он остаётся в дереве и продолжает
получать вызовы 'update' и все другие события жизненного цикла. Он
по-прежнему будет реагировать на события ввода и взаимодействовать с другими
компонентами, например, при обнаружении столкновений.
```

Примесь работает, предотвращая выполнение метода `renderTree`, поэтому если `renderTree` переопределён, следует добавить ручную проверку `isVisible`, чтобы сохранить эту функциональность.
...
### Контексты рендеринга (Render Contexts)

Если нужно, чтобы родительский компонент передавал свойства, специфичные для рендеринга, вниз по дереву дочерних компонентов, можно переопределить свойство `renderContext` у родительского компонента. Можно вернуть пользовательский класс, наследующий от `RenderContext`, а затем использовать `findRenderContext` в дочерних компонентах во время рендеринга. Контексты рендеринга хранятся в стеке и распространяются при обходе дерева для рендеринга.
...
class ChildReadsContext extends Component {
  ...
  void render(Canvas canvas) {
    ...
    // context.value доступен
  }
}
```

Каждый компонент имеет доступ к контексту любого родителя выше по дереву. Если несколько компонентов добавляют контексты, соответствующие выбранному типу `T`, будет возвращён «ближайший» (хотя обычно для каждого компонента создают уникальный тип контекста).


## Эффекты

Flame предоставляет набор эффектов, которые можно применять к определённым типам компонентов. Эти эффекты можно использовать для анимации некоторых свойств компонентов, например позиции или размеров. См. [список доступных эффектов](../effects/effects.md).

Примеры работающих эффектов можно найти в [каталоге примеров эффектов](https://github.com/flame-engine/flame/tree/main/examples/lib/stories/effects).
