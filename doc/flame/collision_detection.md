...
Практически в каждой игре требуется знать, когда объекты касаются или перекрываются. Без обнаружения столкновений игрок проходил бы сквозь стены, пули пролетали бы сквозь врагов, а монеты невозможно было бы собрать. Flame предоставляет встроенную систему обнаружения столкновений, чтобы вы могли сосредоточиться на том, *что происходит* при столкновении объектов, а не писать математику пересечений самостоятельно.

Обнаружение столкновений необходимо в большинстве игр для выявления и реакции на пересечение двух компонентов. Например, стрела попадает во врага или игрок подбирает монету.

В большинстве систем обнаружения столкновений используются так называемые хитбоксы для создания более точных ограничивающих рамок компонентов. Во Flame хитбоксы — это области компонента, которые могут реагировать на столкновения и делать [жестовый ввод](inputs/gesture_input.md#gesturehitboxes) более точным.

Система обнаружения столкновений поддерживает три различных типа фигур, из которых можно строить хитбоксы: многоугольник, прямоугольник и круг. К компоненту можно добавить несколько хитбоксов, формирующих область, которая может использоваться либо для обнаружения столкновений, либо для определения, содержит ли она точку. Последнее очень полезно для точного распознавания жестов. Обнаружение столкновений не определяет, что должно происходить при столкновении двух хитбоксов, поэтому пользователь сам реализует реакцию, например, когда два `PositionComponent` имеют пересекающиеся хитбоксы.

Обратите внимание, что встроенная система обнаружения столкновений не учитывает столкновения между двумя хитбоксами, которые проскакивают друг друга. Это может произойти, когда они движутся очень быстро или `update` вызывается с большим шагом времени (например, если приложение находится в фоне). Такое поведение называется туннелированием.
...
Если вы хотите использовать обнаружение столкновений в своей игре, необходимо добавить примесь `HasCollisionDetection` к вашей игре, чтобы она могла отслеживать компоненты, способные сталкиваться.
...
Теперь, когда вы добавляете `ShapeHitbox` к компонентам, которые затем добавляются в игру, они автоматически проверяются на столкновения.

Вы также можете добавить `HasCollisionDetection` непосредственно к другому `Component` вместо `FlameGame`, например, к `World`, используемому для `CameraComponent`. В этом случае хитбоксы, добавленные в дереве этого компонента, будут сравниваться только с другими хитбоксами в этом поддереве, что позволяет иметь несколько миров с обнаружением столкновений в рамках одного `FlameGame`.
...
```{note}
Хитбоксы будут привязаны только к одной системе обнаружения столкновений — ближайшему родителю с примесью `HasCollisionDetection`.
```


### CollisionCallbacks

Для реакции на столкновение следует добавить примесь `CollisionCallbacks` к вашему компоненту.
Пример:
...
В этом примере мы используем ключевое слово Dart `is`, чтобы проверить, с каким типом компонента произошло столкновение. Множество точек — это точки пересечения границ хитбоксов.

Обратите внимание, что метод `onCollision` будет вызван для обоих `PositionComponent`, если они оба реализовали `onCollision`, а также для обоих хитбоксов. То же относится к методам `onCollisionStart` и `onCollisionEnd`, которые вызываются, когда два компонента и хитбокса начинают или прекращают сталкиваться друг с другом.

Когда `PositionComponent` (и хитбокс) начинает сталкиваться с другим `PositionComponent`, вызываются и `onCollisionStart`, и `onCollision`, поэтому если вам не нужно делать что-то особенное при начале столкновения, можно переопределить только `onCollision`, и наоборот.

Если вы хотите проверять столкновения с краями экрана, как в примере выше, можно использовать предопределённый класс [ScreenHitbox](#screenhitbox).

По умолчанию все хитбоксы полые, то есть один хитбокс может быть полностью заключён в другом, не вызывая столкновения. Чтобы сделать хитбоксы сплошными, установите `isSolid = true`. Полый хитбокс внутри сплошного вызовет столкновение, но не наоборот. Если нет пересечений рёбер со сплошным хитбоксом, вместо этого возвращается его центральная точка.


### Порядок столкновений

Если `Hitbox` сталкивается более чем с одним другим `Hitbox` в течение одного временного шага, обратные вызовы `onCollision` будут вызваны в практически случайном порядке. В некоторых случаях это может быть проблемой, например в игре с прыгающим мячом, где траектория мяча может различаться в зависимости от того, с каким объектом произошло столкновение первым. Чтобы помочь решить эту проблему, можно использовать слушатель `collisionsCompletedNotifier` — он срабатывает в конце процесса обнаружения столкновений.

Пример использования: добавить в ваш `PositionComponent` локальную переменную для сохранения компонентов, с которыми он сталкивается: `List<PositionComponent> collisionComponents = [];`. Затем обратный вызов `onCollision` используется для сохранения всех остальных `PositionComponent` в этот список:
...
Наконец, в метод `onLoad` компонента добавляется слушатель, вызывающий функцию для обработки столкновений:
...
Список `collisionComponents` нужно очищать при каждом вызове `update`.


## ShapeHitbox

`ShapeHitbox` — это обычные компоненты, поэтому вы добавляете их к компоненту, для которого хотите создать хитбоксы, как и любой другой компонент:
...
Если не передавать аргументы в хитбокс, как выше, он попытается максимально заполнить своего родителя. Помимо автоматического заполнения, есть два способа инициализации хитбоксов. Один — через обычный конструктор, где вы задаёте хитбокс отдельно с размером и позицией и т.д. Другой способ — использовать `relative` конструктор, который определяет хитбокс относительно размера предполагаемого родителя.

В некоторых специфических случаях может потребоваться обрабатывать столкновения только между хитбоксами, не передавая события `onCollision*` родительскому компоненту. Например, у автомобиля может быть хитбокс кузова для контроля столкновений и боковые хитбоксы для проверки возможности поворота налево или направо. Столкновение с хитбоксом кузова означает столкновение с самим компонентом, тогда как столкновение с боковым хитбоксом не является настоящим столкновением и не должно передаваться родителю. Для этого можно установить переменную `triggersParentCollision` в `false`:
...
class MyComponent extends PositionComponent {
  ...
  void update(double dt) {
    if (utilityHitbox.isColliding) {
      // выполнить особые действия, когда хитбокс сталкивается
    }
  }
  // функции onCollision* компонента, игнорирующие столкновения MySpecialHitbox.
}
...
class MySpecialHitbox extends RectangleHitbox {
  ...
  // специфичные для хитбокса функции onCollision*
}
```

Подробнее о том, как определяются различные фигуры, читайте в разделе [ShapeComponents](components/shape_components.md).

Помните, что вы можете добавить сколько угодно `ShapeHitbox` к `PositionComponent`, чтобы сформировать более сложные области. Например, снеговика в шляпе можно представить тремя `CircleHitbox` и двумя `RectangleHitbox` в качестве шляпы.

Хитбокс может использоваться либо для обнаружения столкновений, либо для повышения точности распознавания жестов поверх компонентов, подробнее см. в разделе о [GestureHitboxes](inputs/gesture_input.md#gesturehitboxes).


### CollisionType

У хитбоксов есть поле `collisionType`, определяющее, когда хитбокс должен сталкиваться с другим. Обычно рекомендуется устанавливать как можно большему числу хитбоксов значение `CollisionType.passive`, чтобы повысить производительность обнаружения столкновений. По умолчанию `CollisionType` — `active`.

Перечисление `CollisionType` содержит следующие значения:

- `active` сталкивается с другими `Hitbox` типа active или passive
- `passive` сталкивается с другими `Hitbox` типа active
- `inactive` не сталкивается ни с какими другими `Hitbox`

Таким образом, если у вас есть хитбоксы, между которыми не нужно проверять столкновения, вы можете пометить их как passive, указав в конструкторе `collisionType: CollisionType.passive`. Например, это могут быть наземные компоненты, или, возможно, врагам не нужно проверять столкновения между собой, тогда их тоже можно пометить как `passive`.

Представьте игру, где много пуль, которые не могут сталкиваться друг с другом, летят в игрока; тогда игрока можно установить в `CollisionType.active`, а пули — в `CollisionType.passive`.

Тип `inactive` вообще не проверяется при обнаружении столкновений. Это можно использовать, например, если у вас есть компоненты за пределами экрана, которые в данный момент не важны, но позже могут вернуться в поле зрения, так что они не удаляются полностью из игры.

Это лишь примеры использования этих типов; существует гораздо больше сценариев их применения, так что не стесняйтесь использовать их, даже если ваш случай здесь не описан.


### PolygonHitbox

Следует отметить, что если вы хотите использовать обнаружение столкновений или `containsPoint` с `Polygon`, многоугольник должен быть выпуклым. Поэтому всегда используйте выпуклые многоугольники, иначе вы, скорее всего, столкнётесь с проблемами, если не уверены в своих действиях.

Другие формы хитбоксов не имеют обязательного конструктора, потому что для них можно вычислить значение по умолчанию на основе размера сталкивающегося компонента, к которому они прикреплены. Но поскольку многоугольник можно создать внутри ограничивающего прямоугольника бесконечным числом способов, для этой формы необходимо задать определение в конструкторе.

`PolygonHitbox` имеет те же конструкторы, что и [PolygonComponent](components/shape_components.md#polygoncomponent), см. этот раздел для документации по ним.


### RectangleHitbox

`RectangleHitbox` имеет те же конструкторы, что и [RectangleComponent](components/shape_components.md#rectanglecomponent), см. этот раздел для документации по ним.


### CircleHitbox

`CircleHitbox` имеет те же конструкторы, что и [CircleComponent](components/shape_components.md#circlecomponent), см. этот раздел для документации по ним.


## ScreenHitbox

`ScreenHitbox` — это компонент, представляющий границы вашего вьюпорта/экрана. Если добавить `ScreenHitbox` в игру, другие компоненты с хитбоксами будут уведомлены при столкновении с краями. Он не принимает аргументов, зависит только от `size` игры, в которую добавлен. Чтобы добавить его, можно просто сделать `add(ScreenHitbox())` в вашей игре, если вы не хотите, чтобы сам `ScreenHitbox` получал уведомления о столкновениях с ним. Поскольку `ScreenHitbox` имеет примесь `CollisionCallbacks`, при необходимости вы можете добавить собственные функции `onCollisionCallback`, `onStartCollisionCallback` и `onEndCollisionCallback` к этому объекту.


## CompositeHitbox

В `CompositeHitbox` можно добавить несколько хитбоксов, чтобы они имитировали один объединённый хитбокс.

Если, например, вы хотите создать шляпу, вам может понадобиться два [RectangleHitbox](#rectanglehitbox), чтобы правильно следовать краям шляпы; тогда вы можете добавить эти хитбоксы в экземпляр этого класса и реагировать на столкновения со всей шляпой, а не с каждым хитбоксом по отдельности.


## Широкая фаза (Broad phase)

Если ваше игровое поле не огромно и в нём не очень много сталкивающихся компонентов — вам не нужно беспокоиться о используемой системе широкой фазы, так что если стандартная реализация достаточно производительна для вас, этот раздел можно пропустить.

Широкая фаза — это первый шаг обнаружения столкновений, на котором вычисляются потенциальные столкновения. Расчёт этих потенциальных столкновений выполняется быстрее, чем точная проверка пересечений, и устраняет необходимость проверки всех хитбоксов друг с другом, тем самым избегая сложности O(n²).

Широкая фаза производит набор потенциальных столкновений (набор `CollisionProspect`). Этот набор затем используется для проверки точных пересечений между хитбоксами (иногда это называют «узкой фазой»).

По умолчанию обнаружение столкновений Flame использует широкую фазу sweep and prune. Если вашей игре требуется другой тип широкой фазы, вы можете написать свою, расширив `Broadphase` и вручную указав используемую систему обнаружения столкновений.

Например, если вы реализовали широкую фазу на основе волшебного алгоритма вместо стандартного sweep and prune, нужно сделать следующее:
...
## Широкая фаза на Quad Tree

Если ваше игровое поле велико и игра содержит много сталкивающихся компонентов (более сотни), стандартный sweep and prune может стать неэффективным. В этом случае можно попробовать использовать широкую фазу на Quad Tree.

Для этого добавьте примесь `HasQuadTreeCollisionDetection` к вашей игре вместо `HasCollisionDetection` и вызовите функцию `initializeCollisionDetection` при загрузке игры:
...
При вызове `initializeCollisionDetection` необходимо передать правильные размеры карты, чтобы алгоритм Quad Tree работал корректно. Также есть дополнительные параметры для повышения эффективности:

- `minimumDistance`: минимальное расстояние между объектами, чтобы считать их возможно сталкивающимися. Если `null` — проверка отключена, это поведение по умолчанию.
- `maxObjects`: максимальное количество объектов в одном квадранте. По умолчанию 25.
- `maxDepth`: максимальный уровень вложенности внутри квадранта. По умолчанию 10.

Если вы используете систему Quad Tree, можно сделать её ещё эффективнее, реализовав функцию `onComponentTypeCheck` примеси `CollisionCallbacks` в ваших компонентах. Это полезно, если требуется предотвратить столкновения объектов разных типов. Результат вычисления кешируется, поэтому не следует проверять здесь динамические параметры; функция предназначена для использования как чисто проверка типов:
...
class Bullet extends PositionComponent with CollisionCallbacks {
  ...
  bool onComponentTypeCheck(PositionComponent other) {
    if (other is Player || other is Water) {
      // НЕ сталкиваться с Player или Water
      return false;
    }
    // Просто вернуть true, если не интересует результат проверки родительского типа.
    // Или вызвать super, чтобы переопределить результат родительским результатом.
  ...
  void onCollisionStart(
    ...
    // Удаляет компонент при контакте с Brick.
    // Ни Player, ни Water не попадут в эту функцию,
    // потому что эти классы отфильтрованы [onComponentTypeCheck] на более раннем этапе.
    if (other is Brick) {
      removeFromParent();
    }
...
После интенсивного игрового процесса карта может стать чрезмерно кластеризованной с множеством пустых квадрантов. Запустите `QuadTree.optimize()` для очистки пустых квадрантов:

```dart
class QuadTreeExample extends FlameGame
        with HasQuadTreeCollisionDetection {

  /// Функция, вызываемая, когда заканчивается интенсивная игровая сессия.
  /// Может также выполняться по расписанию, но не нужно вызывать при каждом обновлении.
  /// Используйте подходящий интервал в зависимости от обстоятельств вашей игры.
  onGameIdle() {
...
```{note}
Всегда экспериментируйте с разными подходами к обнаружению столкновений
и проверяйте их производительность в вашей игре.
Нередко `QuadTreeBroadphase` оказывается значительно 
*медленнее* стандартного.
Не стоит полагать, что более сложный подход всегда быстрее.
```


## Рейкастинг и трассировка лучей

Рейкастинг и трассировка лучей — это методы испускания лучей из точки в игре и определения, с чем эти лучи сталкиваются и как они отражаются после попадания.

Для всех нижеперечисленных методов, если есть хитбоксы, которые вы хотите игнорировать, можно передать аргумент `ignoreHitboxes` — список хитбоксов, которые не должны учитываться при вызове. Это может быть весьма полезно, например, если вы испускаете лучи изнутри хитбокса, который может быть на вашем игроке или NPC; или если вы не хотите, чтобы луч отражался от `ScreenHitbox`.


### Рейкастинг

Рейкастинг — это операция испускания одного или нескольких лучей из точки и проверка, попадают ли они во что-либо, в случае Flame — в хитбоксы.

Мы предоставляем два метода для этого: `raycast` и `raycastAll`. Первый просто испускает один луч и возвращает результат с информацией о том, во что и где луч попал, а также дополнительную информацию, такую как расстояние, нормаль и отражённый луч. Второй, `raycastAll`, работает аналогично, но испускает несколько лучей равномерно вокруг начальной точки или в пределах угла с центром в начальной точке.

По умолчанию `raycast` и `raycastAll` ищут ближайшее попадание независимо от расстояния от начала луча. Но в некоторых случаях может быть интересно найти попадания только в пределах определённой дистанции. Для таких случаев можно указать необязательный параметр `maxDistance`.

Чтобы использовать функциональность рейкастинга, ваша игра должна иметь примесь `HasCollisionDetection`. После её добавления вы можете вызвать `collisionDetection.raycast(...)` из класса игры или с помощью примеси `HasGameReference` из других компонентов.
...
В этом примере используется класс `Ray2`, который определяет луч с начальной точкой и направлением (оба задаются `Vector2`). Этот конкретный луч начинается в `0, 100` и направлен строго вправо.

Результатом операции будет либо `null`, если луч ни во что не попал, либо `RaycastResult`, который содержит:

- В какой хитбокс попал луч
- Точку пересечения столкновения
- Отражённый луч, т.е. как луч отразится от хитбокса, в который он попал
- Нормаль столкновения, т.е. вектор, перпендикулярный грани хитбокса, в которую попал луч

Если вас беспокоит производительность, можно предварительно создать объект `RaycastResult` и передать его в метод через аргумент `out`; это позволит методу переиспользовать этот объект вместо создания нового при каждой итерации. Это может быть полезно при большом количестве рейкастингов в методах `update`.


#### raycastAll

Иногда требуется испустить лучи во всех направлениях или в ограниченном диапазоне из начальной точки. Это может иметь множество применений, например, расчёт поля зрения игрока или врага, или создание источников света.
...
В этом примере мы испускаем 100 лучей из точки (200, 200), равномерно распределённых во всех направлениях.

Если нужно ограничить направления, используйте аргументы `startAngle` и `sweepAngle`. `startAngle` (отсчитывая от направления вверх) задаёт начало лучей, а лучи будут заканчиваться на `startAngle + sweepAngle`.

Если вас беспокоит производительность, можно повторно использовать объекты `RaycastResult`, создаваемые функцией, передав их список через аргумент `out`.


### Трассировка лучей

Трассировка лучей похожа на рейкастинг, но вместо простой проверки, во что попал луч, вы можете продолжить отслеживать луч и увидеть, во что попадёт его отражённый луч (луч, отскочивший от хитбокса), а затем во что попадёт отражение отражения, и так далее, пока не решите, что достаточно долго трассировали луч. Представьте, например, как бильярдный шар будет отскакивать от бортов стола; эту информацию можно получить с помощью трассировки лучей.
...
В примере выше мы испускаем луч из (0, 100) по диагонали вправо вниз и указываем, что он должен отразиться не более чем от 100 хитбоксов. Не обязательно получится 100 результатов, потому что в какой-то момент один из отражённых лучей может не попасть ни в один хитбокс, и тогда метод завершится.

Метод ленивый, то есть вычисления выполняются только по мере запроса, поэтому нужно итерироваться по возвращаемому итератору, чтобы получить результаты, или вызвать `toList()`, чтобы сразу вычислить все результаты.

В цикле for видно, как это можно использовать: в этом цикле мы проверяем, находится ли точка пересечения текущего отражённого луча (где предыдущий луч попал в хитбокс) дальше чем на 300 пикселей от начальной точки исходного луча, и если да, то остальные результаты нам не нужны (и тогда они не будут вычисляться).
...
Если вы хотите иметь полноценный физический движок в игре, мы рекомендуем использовать Forge2D, добавив [flame_forge2d](https://github.com/flame-engine/flame/tree/main/packages/flame_forge2d) в зависимости. Но если у вас более простая задача и нужно лишь проверять столкновения компонентов и повышать точность жестов, встроенная система обнаружения столкновений Flame вас вполне устроит.

Если вам требуется следующее, стоит по крайней мере рассмотреть использование [Forge2D](https://github.com/flame-engine/forge2d):

- Реалистичное взаимодействие сил
- Системы частиц, взаимодействующие с другими телами
- Сочленения между телами

С другой стороны, разумно использовать только систему обнаружения столкновений Flame, если вам нужно лишь что-то из перечисленного (поскольку это проще, чем подключать Forge2D):

- Возможность реагировать на столкновение некоторых компонентов
- Возможность реагировать на столкновение компонентов с границами экрана
- Сложные формы в качестве хитбокса компонента для более точных жестов
- Хитбоксы, способные определить, какая часть компонента столкнулась с чем-либо
...
- [More Examples](https://github.com/flame-engine/flame/tree/main/examples/lib/stories/collision_detection)# Обнаружение столкновений
...
- [Circles](https://examples.flame-engine.org/#/Collision_Detection_Circles)
- [Multiple shapes](https://examples.flame-engine.org/#/Collision_Detection_Multiple_shapes)
- [More Examples](https://github.com/flame-engine/flame/tree/main/examples/lib/stories/collision_detection)# Collision Detection
