...
В большинстве игр мир больше, чем может уместиться на экране одновременно. Камера управляет тем, какая часть игрового мира видна и как она проецируется на дисплей игрока: панорамирование, масштабирование и следование за персонажами. Это похоже на то, как [`Viewport`](https://api.flutter.dev/flutter/rendering/RenderViewport-class.html) во Flutter определяет, какая часть прокручиваемой области видна, но адаптировано для свободного двумерного координатного пространства игры.
...
Чтобы понять, как работает `CameraComponent`, представьте, что ваш игровой мир — это сущность, существующая *где-то* независимо от вашего приложения. Вообразите, что ваша игра — это всего лишь окно, через которое вы смотрите в этот мир. Вы можете закрыть это окно в любой момент, а игровой мир останется на месте. Или, наоборот, открыть несколько окон, которые одновременно смотрят на один и тот же мир (или на разные миры).

С таким подходом мы теперь можем понять, как работает `CameraComponent`.

Во-первых, есть класс [World](#world), который содержит все компоненты, находящиеся в вашем игровом мире. Компонент `World` может быть смонтирован где угодно, например, в корне вашего игрового класса, как встроенный `World`.

Затем — класс [CameraComponent](#cameracomponent), который «смотрит на» [World](#world). Внутри `CameraComponent` находятся [Viewport](#viewport) и [Viewfinder](#viewfinder), обеспечивая гибкость как рендеринга мира в любом месте экрана, так и управления точкой обзора и углом. `CameraComponent` также содержит компонент [backdrop](#backdrop), который статически отрисовывается позади мира.


## World

Этот компонент предназначен для размещения всех остальных компонентов, составляющих ваш игровой мир. Главная особенность класса `World` в том, что он не рендерится традиционными средствами; вместо этого он отрисовывается одним или несколькими компонентами [CameraComponent](#cameracomponent), которые «смотрят» на мир. В классе `FlameGame` существует один `World` с именем `world`, который добавляется по умолчанию и связан с камерой по умолчанию `CameraComponent`, называемой `camera`.

В игре может быть несколько экземпляров `World`, которые можно отображать одновременно или в разное время. Например, если у вас есть два мира A и B и одна камера, то переключение цели этой камеры с A на B мгновенно переключит вид на мир B без необходимости размонтировать A и монтировать B.

Как и с большинством `Component`, дочерние компоненты можно добавлять в `World` с помощью аргумента `children` в конструкторе или методов `add` и `addAll`.

Для многих игр желательно расширить мир и создавать свою логику в нём. Такая структура игры может выглядеть так:
...
class MyWorld extends World {
  ...
  Future<void> onLoad() async {
    // Загрузить все ресурсы, необходимые в этом мире
    // и добавить компоненты и т.д.
  }
}
```
...
Это компонент, через который отрисовывается `World`. Несколько камер могут одновременно наблюдать за одним и тем же миром.

В классе `FlameGame` есть `CameraComponent` по умолчанию с именем `camera`, которая связана с `world` по умолчанию, поэтому вам не нужно создавать или добавлять свою `CameraComponent`, если это не требуется вашей игре.

Внутри `CameraComponent` находятся два других компонента: [Viewport](#viewport) и [Viewfinder](#viewfinder). Эти компоненты всегда являются дочерними для камеры.

Класс `FlameGame` имеет поле `camera` в своём конструкторе, поэтому вы можете указать желаемый тип камеры по умолчанию, например камеру с [фиксированным разрешением](#cameracomponent-with-fixed-resolution):
...
Также существует статическое свойство `CameraComponent.currentCamera`, возвращающее объект камеры, которая в данный момент выполняет рендеринг. Это необходимо только для некоторых продвинутых случаев, когда отрисовка компонента зависит от настроек камеры. Например, некоторые компоненты могут решить не отрисовывать себя и свои дочерние элементы, если они находятся за пределами видимой области камеры.


### CameraComponent с фиксированным разрешением

Этот именованный конструктор позволяет имитировать фиксированное разрешение устройства пользователя по вашему выбору. Например:
...
Это создаст камеру с viewport, центрированным по середине экрана, занимающим максимально возможное пространство при сохранении соотношения сторон 4:3 (800x600) и показывающим область игрового мира размером 800x600.

С «фиксированным разрешением» работать очень просто, но оно будет недоиспользовать доступное экранное пространство пользователя, если его устройство случайно не имеет то же соотношение сторон, что и выбранные вами размеры.


## Viewport

`Viewport` — это окно, через которое виден `World`. Это окно имеет определённый размер, форму и положение на экране. Доступно несколько видов viewport, и вы всегда можете реализовать свой собственный.

`Viewport` — это компонент, а значит, в него можно добавлять другие компоненты. Эти дочерние компоненты будут подвержены влиянию позиции viewport, но не его маски отсечения. Таким образом, если viewport — это «окно» в игровой мир, то его дочерние элементы — это то, что можно поместить поверх этого окна.

Добавление элементов во viewport — удобный способ реализации компонентов «HUD».

Доступны следующие viewport:

- `MaxViewport` (по умолчанию): этот viewport расширяется до максимального размера, допустимого игрой, т.е. будет равен размеру игрового холста.
- `FixedResolutionViewport`: сохраняет фиксированными разрешение и соотношение сторон, с чёрными полосами по бокам, если оно не совпадает.
- `FixedSizeViewport`: простой прямоугольный viewport с предопределённым размером.
- `FixedAspectRatioViewport`: прямоугольный viewport, который расширяется, чтобы вместиться в игровой холст, сохраняя своё соотношение сторон.
- `CircularViewport`: viewport в форме круга фиксированного размера.

Если вы добавите дочерние компоненты во `Viewport`, они будут отображаться как статичные элементы HUD перед миром.


## Viewfinder

Эта часть камеры отвечает за то, на какую точку в игровом мире мы в данный момент смотрим. `Viewfinder` также управляет уровнем масштабирования и углом поворота вида.

Свойство `anchor` видоискателя позволяет указать, какая точка внутри viewport служит «логическим центром» камеры. Например, в экшн-играх с боковой прокруткой часто камера фокусируется на главном герое, который отображается не по центру экрана, а ближе к нижнему левому углу. Это смещённое положение будет «логическим центром» камеры, управляемым через `anchor` видоискателя.

Если добавить дочерние компоненты в `Viewfinder`, они будут отображаться перед миром, но позади viewport и с теми же трансформациями, что применяются к миру, поэтому эти компоненты не статичны.

В видоискатель также можно добавлять поведенческие компоненты, например [эффекты](effects.md) или другие контроллеры. Если вы, например, добавите `ScaleEffect`, то сможете добиться плавного масштабирования в игре.


## Backdrop

Для добавления статичных компонентов позади мира можно добавить их в компонент `backdrop` или заменить сам `backdrop`. Это полезно, например, если вы хотите разместить статичный `ParallaxComponent` позади мира, в котором перемещается игрок.
...
1. Использовать функции камеры, такие как `follow()`, `moveBy()` и `moveTo()`. Внутри этот подход использует те же эффекты/поведения, что и (2).

2. Применить эффекты и/или поведения к `Viewfinder` или `Viewport` камеры. Эффекты и поведения — это особые виды компонентов, предназначенные для изменения какого-либо свойства компонента с течением времени.

3. Сделать это вручную. Вы всегда можете переопределить метод `CameraComponent.update()` (или аналогичный метод в видоискателе или viewport) и в нём изменять положение или масштаб видоискателя по своему усмотрению. В некоторых обстоятельствах этот подход может быть приемлем, но в целом он не рекомендуется.

У `CameraComponent` есть несколько методов для управления её поведением:

- `follow()` заставляет камеру следовать за указанной целью. При необходимости можно ограничить максимальную скорость движения камеры или разрешить ей двигаться только по горизонтали/вертикали.

- `stop()` отменяет действие предыдущего вызова и останавливает камеру в её текущем положении.
...
- `setBounds()` позволяет добавить ограничения на допустимые перемещения камеры. Эти ограничения задаются в виде `Shape`, обычно это прямоугольник, но может быть и любая другая фигура.


### visibleWorldRect

Камера предоставляет свойство `visibleWorldRect` — прямоугольник, описывающий область мира, видимую в данный момент через камеру. Эту область можно использовать, чтобы избегать рендеринга компонентов, находящихся вне поля зрения, или реже обновлять объекты, находящиеся далеко от игрока.

`visibleWorldRect` — кешируемое свойство, и оно автоматически обновляется при перемещении камеры или изменении размера viewport.


### canSee

У `CameraComponent` есть метод `canSee`, который можно использовать для проверки, виден ли компонент с точки зрения камеры. Это полезно, например, для отсечения компонентов, не попадающих в поле зрения.
...
[Постобработка](rendering/post_processing.md) — это техника, используемая в разработке игр для применения визуальных эффектов к дереву компонентов после его рендеринга. Её можно добавить к камере через свойство `postProcess`.
...
Подробнее об этом читайте в разделе [Постобработка](rendering/post_processing.md).# Camera & World
