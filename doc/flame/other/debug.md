...
## Возможности FlameGame

Flame предоставляет некоторые отладочные функции для класса `FlameGame`. Эти функции включаются, когда свойство `debugMode` установлено в `true` (или переопределено как `true`).
При включённом `debugMode` каждый `PositionComponent` будет отрисован с отображением своих границ, а на экране будут выведены их координаты. Таким образом, можно визуально проверить границы и положение компонентов.

Посмотрите [рабочий пример отладочных возможностей `FlameGame`](https://github.com/flame-engine/flame/blob/main/examples/lib/stories/components/debug_example.dart).


## Расширение Devtools

Если вы откроете [Flutter DevTools](https://docs.flutter.dev/tools/devtools/overview), то увидите новую вкладку «Flame». На этой вкладке будет отображаться информация о текущей игре, например, визуализация дерева компонентов, возможность запуска, паузы и пошагового выполнения игры, информация о выбранном компоненте и многое другое.


## FPS

Значение FPS, сообщаемое Flame, может быть немного ниже того, что показывают, например, Flutter DevTools, в зависимости от целевой платформы. Источником истины о том, сколько FPS работает в вашей игре, должно быть значение, которое сообщаем мы, поскольку именно им ограничен наш игровой цикл.


### FpsComponent

`FpsComponent` можно добавить в любое место дерева компонентов, и он будет отслеживать, сколько кадров в секунду (FPS) игра отрисовывает в данный момент. Если нужно отобразить это в виде текста в игре, используйте [](#fpstextcomponent).


### FpsTextComponent

`FpsTextComponent` — это просто [TextComponent], оборачивающий `FpsComponent`, поскольку чаще всего требуется отображать текущий FPS где-либо при использовании `FpsComponent`.
...
`ChildCounterComponent` — это компонент, который раз в секунду отображает количество дочерних компонентов типа `T` для заданного компонента (`target`).
Например, следующий код покажет количество `SpriteAnimationComponent`, которые являются дочерними элементами игрового `world`:
...
Этот компонент позволяет разработчикам отслеживать время, проведённое внутри своего кода. Это может быть полезно для отладки производительности участков кода.

Чтобы использовать его, добавьте его куда-нибудь в игру (поскольку это отладочная функция, рекомендуем добавлять компонент только в отладочной сборке/варианте):

```dart
add(TimeTrackComponent());
```

Затем в участке кода, время выполнения которого вы хотите отследить, сделайте следующее:
...
С этими вызовами добавленный `TimeTrackComponent` будет показывать прошедшее время в микросекундах.# Debug features
